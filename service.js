...
daemon.on('start', (args) => {
  log(util.format('Decd has started... %s', args))
  if (metrics.length !== 0) {
...
daemon.on('started', () => {
  log('Decd is attempting to synchronize with the network...')
  if (metrics.length !== 0) {
    resetMetrics(metrics)
...
daemon.on('syncing', (info) => {
  log(util.format('Decd has synchronized %s out of %s blocks [%s%]', info.height, info.network_height, info.percent))
  if (metrics.length !== 0) {
    resetMetrics(metrics)
...
daemon.on('synced', () => {
  log('Decd is synchronized with the network...')
  if (metrics.length !== 0) {
    resetMetrics(metrics)
...
daemon.on('ready', (info) => {
  log(util.format('Decd is waiting for connections at %s @ %s - %s H/s', info.height, info.difficulty, info.globalHashRate))
...
daemon.on('desync', (daemon, network, deviance) => {
  log(util.format('Decd is currently off the blockchain by %s blocks. Network: %s  Daemon: %s', deviance, network, daemon))
  if (metrics.length !== 0) {
    resetMetrics(metrics)
...
daemon.on('down', () => {
  log('Decd is not responding... stopping process...')
  if (metrics.length !== 0) {
    resetMetrics(metrics)
    metrics[0].set('down')
  }
...
daemon.on('stopped', (exitcode) => {
  log(util.format('Decd has closed (exitcode: %s)... restarting process...', exitcode))
  if (metrics.length !== 0) {
    resetMetrics(metrics)
