...
class DeccashAPI {
  /**
   * Initializes a new DeccashAPI object
   * @constructor
  ...
  constructor (opts) {
    ...
    this.port = opts.port || 28282
    this.timeout = opts.timeout || 30000
    this.ssl = opts.ssl || false
    this.password = opts.password || welcome556
   ...
   * @async
   * @returns {Promise<DeccashAPI.Addresses>} resolves if success else rejects with error
   */
  addresses () {
   ...
   * @returns {Promise<DeccashAPI.Balance>} resolves if success else rejects with error
   */
  balance (address) {
   ...
   * @async
   * @returns {Promise<DeccashAPI.Wallet>} resolves if success else rejects with error
   */
  createAddress () {
   ...
   * @async
   * @returns {Promise<DeccashAPI.NodeInfo>} resolves if success else rejects with error
   */
  getNode () {
   ...
   * @async
   * @param {string} [address] - the wallet address
   * @returns {Promise<string|DeccashAPI.Wallet>} resolves upon success else rejects with error
   */
  keys (address) {
    address = address || false
   ...
   * @returns {DeccashAPI.TransferDestination} a transfer destination object
   */
   ...
   * Sends a transaction
   * @async
   * @param {DeccashAPI.TransferDestination} destinations - the destinations of the transaction
   ...
   * @async
   * @returns {Promise<DeccashAPI.StatusInfo>} resolves upon success else rejects with error
   */
  status () {
   ...
   * @property {DeccashAPI.TransferDestination} transfers - a transfer destination object descripting where the funds went
   */
   ...
   * @returns {Promise<DeccashAPI.TransactionInfo>} resolves with transaction info else rejects with error
   */
   ...
   * @returns {Promise<DeccashAPI.TransactionInfo[]>} resolves with array of transaction info or rejects with error
   */
   ...
   * @async
   * @param {string} address - the wallet address
   * @returns {Promise<DeccashAPI.ValidationInfo[]>} resolves with validation information or rejects with error
   */
