...
const DecdRPC = require('deccash-rpc').Decd
...
const Decd = function (opts) {
  opts = opts || {}
  if (!(this instanceof Decd)) return new Decd(opts)
  ...
  */
  ...
  // Begin Decd options
  this.path = opts.path || path.resolve(__dirname, './decd' + ((os.platform() === 'win32') ? '.exe' : ''))
  this.dataDir = opts.dataDir || path.resolve(os.homedir(), './.Deccash')
  this.logFile = opts.logFile || path.resolve(__dirname, './Decd.log')
  ...
  this.rpcBindPort = opts.rpcBindPort || 28287
...
inherits(Decd, EventEmitter)

Decd.prototype.start = function () {
  ...
  this.emit('info', 'Attempting to start deccash-daemon-ha...')
...
Decd.prototype.write = function (data) {
  this._write(util.format('%s\r', data))
}

Decd.prototype._checkChildStdio = function (data) {
...
Decd.prototype._triggerDown = function () {
...
Decd.prototype._triggerUp = function () {
...
Decd.prototype._checkServices = function () {
  if (!this.synced) {
    this.synced = true
...
Decd.prototype._checkRpc = function () {
  return Promise.all([
    this.api.info(),
    this.api.height()
  ])
...
Decd.prototype._checkDaemon = function () {
...
Decd.prototype._write = function (data) {
  this.child.write(data)
}

Decd.prototype._buildargs = function () {
  var args = ''
  ...
  if (this.enableBlockExplorerDetailed) args = util.format('%s --enable-blockexplorer', args)
  if (this.loadCheckpoints) {
  ...
  if (this.dbCompression) args = util.format('%s ', args)
...
Decd.prototype._setupAPI = function () {
  this.api = new DecdRPC({
    host: this.rpcBindIp,
...
Decd.prototype._setupWebSocket = function () {
...
Decd.prototype._registerWebSocketClientEvents = function (socket) {
  var that = this
